...
**Whale 4 — cap the retry sleep without faking the retry.** `scripts/_seed-utils.mjs:817` defines `withRetry(fn, maxRetries = 3, delayMs = 1000)`. The `seed-conflict-intel` suite stubs `fetch` as a persistent 429, so each failing Redis write chain slept 1s then 2s — about 12s per test of pure idle. `WM_SEED_RETRY_DELAY_MS` (`scripts/_seed-utils.mjs:841-843`) now caps *only* the sleep: `const pause = Number.isFinite(overrideMs) ? Math.min(wait, overrideMs) : wait`. The attempt count, the computed `wait` (including the `Retry-After` maximum at `:833-834`), and the warning logged at `:836` all stay real, so anything asserting on attempts or log text is untouched. It is set by exactly one file — `tests/seed-conflict-intel-no-source-exit0.test.mjs:36` — and deliberately not by `tests/seed-utils-with-retry.test.mjs`, whose timing assertions still measure genuine backoff. That file went from ~37s to 6.5s, 21/21 passing.
