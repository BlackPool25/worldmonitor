...
| Name | Type | Required | Description |
|------|------|---------:|-------------|
| `country_code` | string | no | ISO 3166-1 alpha-2, or `WORLD`. Empty returns the unfiltered snapshot. |
| `commodity` | string | no | `wheat`, `corn`, `rice`, `soybeans`, `barley`, or `palmOil`. Empty = all. |
...
- **Freshness budget:** latest marketing year present (WASDE monthly cycle; 60-day fetch-age / 90-day content-age).
