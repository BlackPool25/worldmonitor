...
Per-company corporate intelligence from SEC EDGAR and market data (#5695). Company identity resolves through the SEC's own ticker/name registry to a CIK — by exact ticker, or by a case-insensitive exact SEC title that maps to a single CIK (no prefix guessing). Companies that do not resolve return empty envelopes with `sources: []`; use `unavailable` to tell a registry outage from not-found. There is no domain lookup, because no field SEC publishes could confirm one. Four views multiplex the four backing REST routes.
