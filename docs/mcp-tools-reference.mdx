...
| `limit` | integer | no | Maximum matches. The route returns 20 when omitted and caps at 64 |

- **API endpoint:** `GET /api/intelligence/v1/search-intel-history`
...
| `limit` | integer | no | Maximum precedents. The route returns 10 when omitted and caps at 32 |

- **API endpoint:** `GET /api/intelligence/v1/get-similar-events`
