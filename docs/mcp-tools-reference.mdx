...
**Reading a zero.** `stocksToUse: 0` is ambiguous on the wire. It means *unknown*
when `source` is `faostat` (a production-only gap-fill row, where every stocks
field is a placeholder) or when `totalUseTmt` is `0`; it is a genuine 0% only when
`source` is `psd` **and** `totalUseTmt > 0`. `totalUseTmt` is consumption + exports
for a country, but consumption only for `WORLD`, because world exports are
internal transfers already counted in the importing country's consumption.
