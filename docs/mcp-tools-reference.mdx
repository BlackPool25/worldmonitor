...
Trending keyword, CVE, and APT/FIN threat-group spikes versus baseline, using the same term-candidacy and spike-decision math as the dashboard's trending-keywords engine (minimum recent count, strict baseline multiplier, source-diversity gate). The baseline derives from the story accumulator as a per-window story rate — not the dashboard's incremental 7-day client-side history. The accumulator retains 48 hours, but a high-volume feed can fill the per-call story cap with a shorter, more recent span, so the response reports the hours actually sampled in `baseline_hours` and sets `sample_truncated: true` when the cap bounded the read. Read those fields rather than assuming 48. Results are cached for 10 minutes per `(window_hours, min_count)` combination.
