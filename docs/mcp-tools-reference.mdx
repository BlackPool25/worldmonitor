...
- **Kind:** bounded canonical-route proxy over an LLM classifier. This op was previously parity-excluded as `llm-passthrough`; exposing it behind the standard MCP daily quota (50 quota-consuming calls/UTC day for OAuth contexts) bounds its per-call LLM cost, and the 24h per-title cache absorbs repeats.
...
Current topic clusters computed over the live headline digest with the same Jaccard clustering (0.5 title-token similarity) the dashboard uses, so agents see the same story groupings as the UI. Each cluster reports its primary headline, member count, distinct sources, top keywords (stop-word and generic-term filtered), aggregated threat level/category, and time span. Server-side primary selection is recency-based because digest items carry no per-source tier.
...
| `min_sources` | integer | Only return clusters with at least this many member headlines (1-10). Defaults to 1. |
...
Trending keyword, CVE, and APT/FIN threat-group spikes versus baseline, using the same term-candidacy and spike-decision math as the dashboard's trending-keywords engine (minimum recent count, strict baseline multiplier, source-diversity gate). The baseline derives from the 48-hour story accumulator as a per-window story rate — not the dashboard's incremental 7-day client-side history — and the tool description says so honestly. Results are cached for 10 minutes per `(window_hours, min_count)` combination.
...
- **Kind:** deterministic local compute with a 10-minute Redis result cache. `note` is present when the accumulator is unavailable or empty.
- **Quota:** standard — every call consumes the MCP daily reservation.
