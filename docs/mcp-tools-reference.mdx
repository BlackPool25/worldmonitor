...
Per-company corporate intelligence from SEC EDGAR and market data (#5695). Company identity resolves through the SEC's own ticker/name registry to a CIK — by exact ticker, or by a name that identifies exactly one filer. Companies that do not resolve return empty envelopes with `sources: []`; there is no domain lookup, because no field SEC publishes could confirm one. Four views multiplex the four backing REST routes.
...
| `name` | string | Company name fallback when no ticker is known; resolves only when it identifies exactly one SEC filer. |
