...
Per-company corporate intelligence from SEC EDGAR and market data (#5695). Company identity always resolves through the SEC's own ticker/name registry to a CIK; companies that do not resolve return empty envelopes with `sources: []` — never guessed attribution. Four views multiplex the four backing REST routes.
...
| `name` | string | Company name fallback when no ticker is known; matched against the SEC registry. |
| `domain` | string | Company domain fallback, such as `apple.com`. Accepted only when it matches one SEC filer unambiguously AND that filer registered the same website; otherwise the company resolves to nothing rather than a guess. |
...
| `limit` | integer | `filings-search` (max 25) and `material-events` (max 100) result cap. |
...
- **Kind:** canonical-route proxy — `enrichment`/`signals` fan out per-company (SEC submissions, Finnhub profile + earnings surprises, news mentions) with per-upstream Redis caching; `filings-search` proxies EDGAR full-text search; `material-events` reads the seeded market-wide 8-K stream (30-minute cadence).
