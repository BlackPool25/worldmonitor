...
| [Environment & Disasters](#environment-%26-disasters) | Earthquakes, disasters, wildfires, cyclones, climate anomalies | USGS, CWFIS/NRCan, GDACS, NASA FIRMS/EONET, Open-Meteo |
...
- NASA FIRMS satellite fire detection (VIIRS thermal hotspots) merged with CWFIS national agency-reported active fires; prescribed burns are stored with kind=prescribed and emergency=false (the map and RPC do not yet filter on that flag)
