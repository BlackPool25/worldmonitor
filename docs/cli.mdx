...
description: "Script country briefs, risk scores, and any of the 45 MCP tools from your shell with the official worldmonitor npm CLI — no API integration to write."
---
...
You'll get all 45 tool definitions back as JSON. Everything else that returns *data* is a `tools/call` and needs a user API key.
...
The curated table stays small on purpose — every one of the 45 tools is reachable via `call`, and any unrecognised `--key value` becomes a tool argument, so nothing needs special wiring:
...
- **[MCP Tools Reference](/mcp-tools-reference)** — per-tool parameters, freshness budgets, and examples for all 45 tools.
