...
| Billing portal / checkout | Portal, hosted checkout and every upgrade CTA route through `openExternalUrl`, which hands the URL to the OS browser via `open_url` on desktop; the checkout return URL is built from the canonical web origin, so the buyer finishes and returns in the browser and the app unlocks over the live Convex entitlement watch (no redirect back into the app) | intentional difference (desktop pays in the browser) — #5911 | `src/services/external-navigation.ts`; `src/services/checkout-return-url.ts:resolveCheckoutReturnOrigin`; `src/services/checkout.ts` hosted-checkout branch |
