...
The server exposes 46 tools. Most are cache-reads over pre-seeded Redis keys (sub-second). The slower, non-cache paths are the seven live LLM/external-API tools (`get_world_brief`, `get_country_brief`, `analyze_situation`, `generate_forecasts`, `search_flights`, `search_flight_prices_by_date`, `classify_event`) plus the live geo RPC tools (`get_airspace`, `get_maritime_activity`), the bounded canonical procurement proxy (`get_procurement_opportunities`), and the canonical China decision-signal RPC (`get_china_decision_signals`). The on-demand NLP utilities (`classify_event`, `extract_entities`, `get_news_clusters`, `get_keyword_spikes`) accept bounded caller text or compute over live seeded data; all but `classify_event` are fully deterministic. One (`describe_tool`, added v1.5.0) returns the full uncompressed definition of any other tool — useful when the compressed `tools/list` description is ambiguous; exempt from the Pro daily quota.
...
See the [Tool catalog](#tool-catalog) above for the complete list of 46 tools, or the [MCP Tools Reference](/mcp-tools-reference) for per-tool details.

## Prompts and resources

In addition to the 46 tools, WorldMonitor exposes MCP prompts and resources so clients can discover common workflows and address stable data slices without inventing tool plans from scratch.
