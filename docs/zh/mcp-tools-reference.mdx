...
- **类型：** 对 LLM 分类器的受约束规范路由代理。该操作此前以 `llm-passthrough` 理由被排除在 MCP 对等之外；置于标准 MCP 每日配额之后（OAuth 上下文每 UTC 日 50 次消耗配额的调用）即可约束其单次调用 LLM 成本，且按标题的 24 小时缓存吸收重复请求。
...
使用与仪表盘完全相同的 Jaccard 聚类（0.5 标题分词相似度）对实时头条摘要计算当前话题簇，因此 agent 看到的故事分组与 UI 一致。每个簇报告主标题、成员数、去重来源、热门关键词（过滤停用词与泛化词）、聚合威胁级别/类别以及时间跨度。服务器端主标题按新近度选取，因为摘要条目不携带每来源层级。
...
| `min_sources` | integer | 仅返回成员头条数不低于该值的簇（1-10）。默认 1。 |
...
相对基线的关键词、CVE 与 APT/FIN 威胁组织飙升趋势，使用与仪表盘趋势关键词引擎相同的词项候选与飙升判定数学（最低近期计数、严格基线倍数、来源多样性门槛）。基线源自 48 小时故事累积器的按窗口故事速率 —— 而非仪表盘客户端增量式的 7 天历史 —— 工具描述如实说明了这一点。结果按 `(window_hours, min_count)` 组合缓存 10 分钟。
...
- **类型：** 确定性本地计算，附带 10 分钟 Redis 结果缓存。累积器不可用或为空时返回 `note`。
- **配额：** 标准 —— 每次调用消耗 MCP 每日预留。
