...
相对基线的关键词、CVE 与 APT/FIN 威胁组织飙升趋势，使用与仪表盘趋势关键词引擎相同的词项候选与飙升判定数学（最低近期计数、严格基线倍数、来源多样性门槛）。基线源自故事累积器的按窗口故事速率 —— 而非仪表盘客户端增量式的 7 天历史。累积器保留 48 小时，但高流量信息流可能以更短、更近的时间跨度填满单次调用的故事上限，因此响应会在 `baseline_hours` 中报告实际采样的小时数，并在受上限截断时设置 `sample_truncated: true`。请读取这些字段，而不要假定为 48。结果按 `(window_hours, min_count)` 组合缓存 10 分钟。
