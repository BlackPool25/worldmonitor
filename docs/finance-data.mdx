...
| **Shanghai Gold Exchange (SGE)** | **License-gated physical benchmark source.** SHAU and SHAG ingestion remains disabled unless `SGE_MARKET_DATA_LICENSED=true`. Set that flag only after the deployment's redistribution, display, and attribution rights are documented. |
...
| Physical precious-metals premiums | Daily SGE SHAU/SHAG PM benchmark + existing COMEX and shared CNY/USD snapshots | Daily; 72h run freshness, 10d content-age ceiling | License flag is mandatory; the seeder does not refetch COMEX or FX |
