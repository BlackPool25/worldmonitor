...
Commodity symbols are limited to the configured seed set (see `shared/commodities.json`, e.g. `GC=F` gold, `CL=F` crude oil). The commodity quotes RPC serves only this set — an unsupported symbol is rejected rather than silently dropped, and an empty request returns the full default set.
