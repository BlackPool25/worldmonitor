...
test('PR1 #6806: 1-section arms/bases bundles exist; leftover still hosts both until cutover', () => {
  ...
  const leftoverLabels = leftover.sections.map((s) => s.label);
  assert.ok(leftoverLabels.includes('Arms-Suppliers'), 'D6: leftover still lists Arms-Suppliers');
  assert.ok(leftoverLabels.includes('Military-Bases'), 'D6: leftover still lists Military-Bases');
  assert.ok(
    leftoverLabels.length >= 7,
    `leftover still hosts the pre-cutover membership, found ${leftoverLabels.length}: ${leftoverLabels.join(', ')}`,
  );
});
