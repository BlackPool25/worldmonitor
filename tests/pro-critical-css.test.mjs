...
describe('pro built HTML critical CSS contract', () => {
  for (const { relPath, label } of PRO_PAGES) {
    ...
    it(`${label} re-shows responsive nav/hero reveals so unlayered .hidden can't hide them at all widths`, () => {
      ...
      const navRevealIdx = criticalCss.indexOf('nav [class~="lg:flex"]{display:flex}');
      ...
      assert.notEqual(navRevealIdx, -1, `${relPath} critical CSS must re-show hidden lg:flex nav rows at >=1024px`);
      ...
      assert.ok(navRevealIdx > navHideIdx, `${relPath} nav lg:flex reveal must follow nav .hidden to win the cascade`);
      assert.ok(smBlockRevealIdx > mainHideIdx, `${relPath} main sm:block reveal must follow main .hidden to win the cascade`);
      // The nav reveal must sit inside the >=1024px block (gated to desktop, not applied at all widths).
      assert.ok(lg1024Idx !== -1 && navRevealIdx > lg1024Idx, `${relPath} nav lg:flex reveal must be inside the min-width:1024px media block`);
