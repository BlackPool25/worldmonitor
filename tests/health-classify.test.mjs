...
test('classifyKey: each bundle tick heartbeat goes EMPTY when the cron never fired and STALE when it freezes', () => {
  for (const [name, key] of [
    ...
    assert.equal(dataKey, key, name);
    assert.equal(seedCfg.key, key, name);
    assert.equal(seedCfg.maxStaleMin, 2880, `${name} 48h = 2× daily`);
    assert.equal(ON_DEMAND_KEYS.has(name), false, name);
...
test('the three bundle tick heartbeats stay registered together (#6691 / #6806)', async () => {
  ...
  for (const [name, label, bundlePath, issue] of ticks) {
